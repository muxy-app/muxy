...
@Suite("Notch terminal geometry")
struct NotchTerminalGeometryTests {
    ...
    func maximumSizeAtTopCenter() {
        ...
        let frame = NotchTerminalGeometry.frame(in: display)
    ...
    func emptyDisplay() {
        #expect(NotchTerminalGeometry.frame(in: .zero) == .zero)
    }
    ...
    func physicalTopAndVisibleBounds() {
        ...
        let frame = NotchTerminalGeometry.frame(screenFrame: screen, visibleFrame: visible)

        #expect(frame == NSRect(x: 360, y: 470, width: 720, height: 430))
    }
}

@Suite("Notch terminal active screen resolution")
struct NotchTerminalScreenResolverTests {
    private let screens = [
    ...
    func mouseDisplayWins() {
        let index = NotchTerminalScreenResolver.preferredScreenIndex(
    ...
    func orderedFallbacks() {
        ...
        #expect(NotchTerminalScreenResolver.preferredScreenIndex(
