...
struct GhosttyConfigOverlayLoaderTests {
    ...
    func loadOrder() throws {
        ...
        let result = loader.load(base: base, overridesFilePath: "notch.conf")

        #expect(result == clone)
        #expect(events == ["clone", "load:notch.conf"])
    }
    ...
    func cloneFailure() throws {
        ...
        #expect(loader.load(base: base, overridesFilePath: "notch.conf") == nil)
        #expect(!didLoad)
    }
}
