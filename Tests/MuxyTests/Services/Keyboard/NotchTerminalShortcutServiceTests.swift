...
@Suite("NotchTerminalShortcutService")
@MainActor
struct NotchTerminalShortcutServiceTests {
    ...
    func changeRecoversAfterInitialFailure() throws {
        let initialShortcut = NotchTerminalShortcut.keyCombo(
            KeyCombo(key: "space", command: true),
            virtualKeyCode: 49
        )
    ...
    func serviceDeinitializationStopsBackend() throws {
        let store = makeStore(persistence: ServiceShortcutPersistence())
        let backend = TestShortcutBackend(state: .localOnly)
        var service: NotchTerminalShortcutService? = makeService(
            store: store,
            doubleShiftBackend: backend
        )
        try service?.start()
    ...
    private func makeStore(persistence: ServiceShortcutPersistence) -> NotchTerminalShortcutStore {
        NotchTerminalShortcutStore(persistence: persistence, settingsSynchronizer: {})
    }

    private func makeService(
        store: NotchTerminalShortcutStore,
...
@MainActor
private final class TestShortcutBackend: NotchTerminalShortcutBackend {
    let name: String
    private(set) var monitoringState: NotchTerminalShortcutMonitoringState
    private let enabledState: NotchTerminalShortcutMonitoringState
    ...
    init(
        name: String = "backend",
        state: NotchTerminalShortcutMonitoringState,
        enabledState: NotchTerminalShortcutMonitoringState? = nil,
...
private final class ServiceShortcutPersistence: NotchTerminalShortcutPersisting {
