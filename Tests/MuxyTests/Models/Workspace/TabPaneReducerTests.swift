...
struct TabPaneReducerTests {
    ...
    @Test("closing a missing internal pane does nothing")
    func closingMissingInternalPaneDoesNothing() {
        let projectID = UUID()
        let worktreeID = UUID()
        var state = makeState(projectID: projectID, worktreeID: worktreeID)
        let key = WorktreeKey(projectID: projectID, worktreeID: worktreeID)
        let area = state.workspaceRoots[key]!.allAreas().first!
        let tabID = area.activeTabID!
        _ = TabPaneReducer.splitTabPane(
            projectID: projectID, areaID: area.id, tabID: tabID,
            direction: .horizontal, state: &state
        )
        let paneIDs = area.activeTab!.internalPanes!.allPanes().map(\.id)

        let removed = TabPaneReducer.closeInternalPane(
            projectID: projectID, areaID: area.id, tabID: tabID,
            paneID: UUID(), state: &state
        )

        #expect(removed == nil)
        #expect(area.activeTab!.internalPanes!.allPanes().map(\.id) == paneIDs)
    }
