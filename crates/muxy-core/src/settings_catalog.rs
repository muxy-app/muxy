...
mod tests {
    ...
    fn every_item_key_is_mirrored_or_deliberately_excluded() {
        ...
        let mirrored: Vec<&str> = crate::prefs::settings::MIRROR
            .iter()
            .map(|entry| entry.key)
            .collect();
        ...
        for key in &mirrored {
            assert!(
                super::ITEMS.iter().any(|item| item.key == *key),
                "mirrored key {key} is missing from the catalog"
            );
        }
        assert_eq!(mirrored.len() + EXCLUDED.len(), super::ITEMS.len());
    }
}
