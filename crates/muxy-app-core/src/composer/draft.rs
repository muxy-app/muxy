...
impl DraftId {
    pub fn new(server_id: &str, project_id: &str) -> Option<Self> {
        let server = super::canonical_uuid(server_id)?;
        let project = super::canonical_uuid(project_id)?;
        Some(Self(format!("{server}:{project}")))
    }

    #[allow(
        clippy::expect_used,
        reason = "Typed server and project IDs always format as valid UUIDs"
    )]
    pub fn for_project(server: crate::ServerId, project: crate::ProjectId) -> Self {
        Self::new(&server.to_string(), &project.to_string())
            .expect("Project and server identifiers are UUIDs")
    }

    pub fn parse(value: &str) -> Option<Self> {
        let (server, project) = value.split_once(':')?;
        let id = Self::new(server, project)?;
...
pub struct ComposerStore {
    ...
    submitted_images: HashSet<String>,
...
impl ComposerStore {
    pub fn load_from(profile_root: impl Into<PathBuf>) -> Self {
        ...
        let profile_lock = match super::storage::lock_profile(&profile_root) {
            Ok(lock) => Some(lock),
            Err(error) => {
    ...
    pub fn retain_submitted_images(&mut self, filenames: impl IntoIterator<Item = String>) {
        self.submitted_images.extend(filenames);
    }
    ...
    pub fn replace_draft(&mut self, id: DraftId, draft: ComposerDraft) -> std::io::Result<u64> {
        ...
        if current.draft == draft && current.pending_removals.is_empty() {
    ...
    fn require_lock(&self) -> std::io::Result<()> {
        if self.profile_lock.is_none() {
            return Err(std::io::Error::other(
                "Composer draft storage is unavailable or in use by another Muxy instance",
            ));
        }
        Ok(())
    }

    fn require_editable(&self, id: &DraftId) -> std::io::Result<()> {
        self.require_lock()?;
        if self.malformed.contains_key(id.as_str()) {
            return Err(invalid_draft(
                "This Composer draft could not be loaded. Editing is blocked to preserve the original data.",
            ));
        }
        Ok(())
    }
    ...
    pub fn flush_if_revision(&mut self, revision: u64) -> std::io::Result<bool> {
        ...
        if self.load_status.overwrite_blocked {
            return Err(std::io::Error::new(
                ...
                "Composer draft storage is blocked; the original data has been preserved",
            ));
        }
        let contents = self.serialize()?;
        if contents.len() > super::MAX_DRAFT_BYTES {
            return Err(invalid_draft(
                "Composer drafts exceed the 32 MiB storage limit",
            ));
        }
        if let Err(error) = super::storage::write_private_durable(&self.path, &contents) {
    ...
    fn referenced_filenames(&self) -> HashSet<String> {
        self.drafts
            ...
            .chain(self.submitted_images.iter().cloned())
            .collect()
    }
...
fn load_document(
    ...
    let contents = match super::storage::read_document(path) {
    ...
    for (key, raw) in root {
        ...
        if let Some((id, draft)) = parsed {
            ...
            status.warnings.push(format!(
                "Composer draft {key} could not be loaded; its original data is preserved and editing is blocked"
            ));
...
mod tests {
    ...
    #[test]
    fn malformed_project_drafts_reject_changes_and_preserve_original_data() {
        let profile = tempfile::tempdir().unwrap();
        let path = profile.path().join(super::super::DRAFTS_FILE_NAME);
        let original = serde_json::json!({
            id().as_str(): {"text": "important notes", "fileAttachments": ["relative-path"]}
        });
        std::fs::write(&path, serde_json::to_vec(&original).unwrap()).unwrap();
        let mut store = ComposerStore::load_from(profile.path());
        assert!(!store.load_status().warnings.is_empty());
        assert!(store.edit_content(id(), "new".into(), Vec::new()).is_err());
        assert!(store.replace_draft(id(), ComposerDraft::default()).is_err());
        assert!(store.clear_if_revision(&id(), 0).is_err());
        assert!(store.attach_image(id(), &png(), 0).is_err());
        let other = DraftId::new(PROJECT, OTHER_WORKTREE).unwrap();
        store
            .edit_content(other, "unaffected".into(), Vec::new())
            .unwrap();
        store.flush().unwrap();
        let saved: Value = serde_json::from_slice(&std::fs::read(path).unwrap()).unwrap();
        assert_eq!(saved[id().as_str()], original[id().as_str()]);
    }
    ...
    fn submitted_inline_image_survives_clear_until_the_next_launch() {
        let profile = tempfile::tempdir().unwrap();
        let mut store = ComposerStore::load_from(profile.path());
        store.attach_image(id(), &png(), 0).unwrap();
        let filename = store.draft(&id()).unwrap().image_attachments[0]
            .filename
            .clone();
        store.retain_submitted_images([filename.clone()]);
        let revision = store.draft_revision(&id());
        assert!(store.clear_if_revision(&id(), revision).unwrap());
        store.flush().unwrap();
        assert!(store.image_storage().unwrap().read(&filename).is_ok());
        drop(store);
        let store = ComposerStore::load_from(profile.path());
