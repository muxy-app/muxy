...
pub fn git_reply(verb: &str, reply: GitReply) -> Result<Value, String> {
    Ok(match reply {
        ...
        GitReply::Diff(diff) => json!({"additions":diff.additions,"deletions":diff.deletions,"truncated":diff.truncated,
            "rows":diff.rows.iter().map(|r| json!({"kind":match r.kind {GitDiffKind::Hunk=>"hunk",GitDiffKind::Context=>"context",GitDiffKind::Addition=>"addition",GitDiffKind::Deletion=>"deletion",GitDiffKind::Unrecognized(_)=>"other"},"oldLineNumber":r.old_line_number,"newLineNumber":r.new_line_number,"oldText":r.old_text,"newText":r.new_text,"text":r.text})).collect::<Vec<_>>()}),
