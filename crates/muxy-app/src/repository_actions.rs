...
impl Action {
    pub(crate) fn index(self) -> usize {
        match self {
            Self::Commit => 0,
            Self::CreatePullRequest => 1,
        }
    }

    pub(crate) fn symbol(self) -> &'static str {
        match self {
            Self::Commit => "arrow.up.circle",
            Self::CreatePullRequest => "arrow.triangle.pull",
        }
    }
...
pub(crate) enum Mode {
    Commit,
    /// Pull request changes always move to a new branch.
    NewBranch,
}

/// Repository context and the expected tree for a confirmed action.
...
impl Plan {
    ...
    pub(crate) fn destination(&self, head: &str) -> GitPushDestination {
        if self.mode == Mode::Commit
            && let Some(destination) = &self.preview.destination
        {
...
/// Validated metadata generated by the selected provider.
...
pub(crate) fn prompt(plan: &Plan, instructions: &str) -> String {
    ...
    let (request, shape, guidance) = match plan.action {
        ...
        Action::CreatePullRequest => (
            ...
            "Branch names must not include a remote prefix. Choose targetBranchName from remoteBranches and prefer the provided default branch as the target when appropriate. When pullRequestTemplate is present, follow its structure in the summary.\n",
        ),
    };
    format!(
...
/// Decodes provider metadata without silently changing its branch choices.
    ...
    match plan.action {
        ...
        Action::CreatePullRequest => {
            ...
            let branch = match plan.mode {
                ...
                Mode::Commit => plan.branch.clone(),
            };
            let target = text_field(&metadata, "targetBranchName").to_owned();
...
/// Checks generated metadata before committing or publishing.
