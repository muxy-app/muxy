...
class MuxyClient(
    ...
    fun suspendForBackground() {
        ...
        log.append("Suspending socket for background")
        socket.close(1000, "background")
        webSocket = null
        scope.launch {
            cancelAllPending(MuxyError(code = 499, message = "Suspended for background"))
        }
    }
    ...
    suspend fun fetchProjectLogo(projectID: UUID): Boolean {
        ...
        val data =
            runCatching { Base64.getDecoder().decode(result.value.pngData) }.getOrNull()
                ?: return false
    ...
    private suspend fun authenticateOrPair(
        target: ConnectionTarget,
