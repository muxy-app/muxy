...
class MuxyClientVCSTest {
    ...
    @Test
    fun `fetchVCSStatus returns parsed payload on success`() =
        runBlocking {
    ...
    @Test
    fun `fetchVCSStatus returns null on server error`() =
        runBlocking {
            startAndConnect { request ->
                if (request.method == MuxyMethod.GET_VCS_STATUS) {
                    server.broadcast(
    ...
    @Test
    fun `vcsCommit succeeds when server returns ok`() =
        runBlocking {
    ...
    @Test
    fun `stageFiles throws Server error when Mac responds with error`() =
        runBlocking {
            startAndConnect { request ->
            ...
            val ex =
                assertThrows(VCSClientError.Server::class.java) {
    ...
    @Test
    fun `listBranches returns parsed payload`() =
        runBlocking {
    ...
    @Test
    fun `createPullRequest returns PR result`() =
        runBlocking {
    ...
    @Test
    fun `addWorktree refreshes worktrees on success`() =
        runBlocking {
    ...
    @Test
    fun `selectProject sets activeProjectID and refreshes workspace`() =
        runBlocking {
