...
class MuxyClientTest {
    ...
    @Test
    fun `authenticateDevice success transitions to Connected`() =
        runBlocking {
    ...
    @Test
    fun `401 unauthorized triggers pairing flow then connects`() =
        runBlocking {
            var seenAuth = false
            server.start { incoming ->
                val request = (incoming as MuxyMessage.Request).value
    ...
    @Test
    fun `403 pairingDenied surfaces approval-denied failure`() =
        runBlocking {
            server.start { incoming ->
                val request = (incoming as MuxyMessage.Request).value
                when (request.method) {
    ...
    @Test
    fun `terminalInput is fire-and-forget and does not register pending request`() =
        runBlocking {
            server.start { incoming ->
                val request = (incoming as MuxyMessage.Request).value
            ...
            client.sendFireAndForget(
                ...
                params =
                    MuxyParams.TerminalInput(
                        TerminalInputParams(paneID = paneID, bytes = "hello".toByteArray()),
                    ),
            )
            delay(150)
            val terminalInputs =
                server.receivedMessages().filter { msg ->
    ...
    @Test
    fun `RPC round-trip listProjects returns projects payload`() =
        runBlocking {
            val project =
                ProjectDTO(
                    id = UUID.randomUUID(),
    ...
    @Test
    fun `RPC times out and returns 408 when server never responds`() =
        runBlocking {
            server.start { incoming ->
                val request = (incoming as MuxyMessage.Request).value
                if (request.method == MuxyMethod.AUTHENTICATE_DEVICE) {
    ...
    @Test
    fun `paneOwnershipChanged event updates paneOwners map`() =
        runBlocking {
            server.start { incoming ->
                val request = (incoming as MuxyMessage.Request).value
                if (request.method == MuxyMethod.AUTHENTICATE_DEVICE) {
    ...
    @Test
    fun `terminalOutput and terminalSnapshot both flow through terminalBytes for that pane`() =
        runBlocking {
            server.start { incoming ->
                val request = (incoming as MuxyMessage.Request).value
                if (request.method == MuxyMethod.AUTHENTICATE_DEVICE) {
            ...
            server.broadcast(
                MuxyMessage.Event(
                    MuxyEvent(
                        ...
                        data =
                            MuxyEventData.TerminalSnapshot(
    ...
    @Test
    fun `verifyConnectionOrReconnect re-authenticates and clears paneOwners`() =
        runBlocking {
            var authCount = 0
            server.start { incoming ->
                val request = (incoming as MuxyMessage.Request).value
    ...
    @Test
    fun `disconnect cancels pending requests with cancellation error`() =
        runBlocking {
            server.start { incoming ->
                val request = (incoming as MuxyMessage.Request).value
                if (request.method == MuxyMethod.AUTHENTICATE_DEVICE) {
