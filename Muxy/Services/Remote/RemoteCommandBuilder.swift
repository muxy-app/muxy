...
enum RemoteCommandBuilder {
    ...
    static func containmentGuardPrefix(root: String, target: String) -> String {
        ...
        return "__muxy_root=$(cd \(quotedRoot) 2>/dev/null && { pwd -P; printf '\\001'; }) "
            ...
            + "__muxy_resolve() { "
            + "__muxy_path=$1; __muxy_suffix=; __muxy_hops=0; "
            + "case \"$__muxy_path\" in /*) ;; *) __muxy_path=\"$PWD/$__muxy_path\" ;; esac; "
            + "while :; do "
            + "while [ ! -e \"$__muxy_path\" ] && [ ! -L \"$__muxy_path\" ]; do "
            + "[ \"$__muxy_path\" != / ] || return 1; "
            + "__muxy_name=${__muxy_path##*/}; __muxy_parent=${__muxy_path%/*}; "
            + "[ -n \"$__muxy_parent\" ] || __muxy_parent=/; "
            + "__muxy_suffix=\"/$__muxy_name$__muxy_suffix\"; __muxy_path=$__muxy_parent; "
            + "done; "
            + "if [ -L \"$__muxy_path\" ]; then "
            + "__muxy_link=$(readlink -n \"$__muxy_path\" && printf '\\001') || return 1; "
            + "__muxy_link=${__muxy_link%?}; "
            + "__muxy_hops=$((__muxy_hops + 1)); [ \"$__muxy_hops\" -le 40 ] || return 1; "
            + "case \"$__muxy_link\" in "
            + "/*) __muxy_path=$__muxy_link ;; "
            + "*) __muxy_parent=${__muxy_path%/*}; [ -n \"$__muxy_parent\" ] || __muxy_parent=/; "
            + "__muxy_path=$__muxy_parent/$__muxy_link ;; "
            + "esac; "
            + "__muxy_path=\"$__muxy_path$__muxy_suffix\"; __muxy_suffix=; continue; "
            + "fi; "
            + "if [ -d \"$__muxy_path\" ]; then "
            + "__muxy_directory=$(cd \"$__muxy_path\" 2>/dev/null && { pwd -P; printf '\\001'; }) "
            + "|| return 1; "
            + "__muxy_directory=${__muxy_directory%?}; __muxy_directory=${__muxy_directory%?}; "
            + "printf '%s%s\\001' \"$__muxy_directory\" \"$__muxy_suffix\"; return; "
            + "fi; "
            + "__muxy_name=${__muxy_path##*/}; __muxy_parent=${__muxy_path%/*}; "
            + "[ -n \"$__muxy_parent\" ] || __muxy_parent=/; "
            + "__muxy_directory=$(cd \"$__muxy_parent\" 2>/dev/null && { pwd -P; printf '\\001'; }) "
            + "|| return 1; "
            + "__muxy_directory=${__muxy_directory%?}; __muxy_directory=${__muxy_directory%?}; "
            + "printf '%s/%s%s\\001' \"$__muxy_directory\" \"$__muxy_name\" \"$__muxy_suffix\"; "
            + "return; "
            + "done; "
            + "}; "
