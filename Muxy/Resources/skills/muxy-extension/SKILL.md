...
- **A quick HTML form or rich prompt on a keypress** → a **webview modal** (`muxy.modal.openWebview`, page-only). A centered omnibox-style overlay that renders your own HTML; the modal submits a value back to the opener's `await`. Reach for this over the native `muxy.modal.open` picker when you need inputs, layout, or validation the list picker can't express.
