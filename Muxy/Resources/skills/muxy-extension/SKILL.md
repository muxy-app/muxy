...
- **Drive the built-in browser with `muxy.browser.*`** to show a web page in a tab and read it back. `open(url, { split })` returns the new tab ID; `navigate(tabId, url)`, `list()`, `read(tabId)` (resolves `{ title, url, text }` of the rendered page), and `close(tabId)`. Reading needs `browser:read`; opening/navigating/closing need `browser:write`. The calls fail when the user has disabled the built-in browser. Capture the tab ID from `open`/`list` and reuse it. See [Browser](https://muxy.app/docs/extensions/browser).
