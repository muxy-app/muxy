...
- **Workspaces can be remote.** When the active workspace is a remote (SSH) workspace, `muxy.exec`, `muxy.execStream`, `muxy.git.*`, and worktree work run **on the remote server** with the selected SSH device's environment, and paths are remote paths. Write extensions against the active workspace, not a hardcoded local machine — the same code works for local and remote because Muxy brokers the SSH connection. See [Scripts](https://muxy.app/docs/extensions/scripts).
- **Use `muxy.execStream` for large command output** such as `rg` search results. It uses the same argv/shell forms and `commands:exec` permission as `muxy.exec`, calls `onStdout(line)` / `onStderr(line)` with complete UTF-8 lines, and supports `maxLines` plus cancellation so extensions can stop the process at a UI result cap.
...
- **Drive the built-in browser with `muxy.browser.*`** to show a web page in a tab and read it back. `open(url, { split })` returns the new tab ID; `navigate(tabId, url)`, `list()`, `read(tabId)` (resolves `{ title, url, text }` of the rendered page, text capped at ~1 MB), and `close(tabId)`. Reading needs `browser:read`; opening/navigating/closing need `browser:write`. Every call fails when the user has disabled the built-in browser. `read` only works for tabs in the active project and waits briefly for the view, so pause after navigating before reading. Capture the tab ID from `open`/`list` and reuse it. See [Browser](https://muxy.app/docs/extensions/browser).
