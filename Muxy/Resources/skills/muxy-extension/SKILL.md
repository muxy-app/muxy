...
- **Use `muxy.modal.open` for a list picker** (the native searchable picker overlay) instead of building your own — pass `{ items: [{ id, title, subtitle? }], placeholder? }` and get the selected item (or `null` if dismissed). Muxy owns the search, navigation, and open/close. No permission needed. Available on every surface: `await` it on pages/background; in a `runScript` command it returns synchronously, so a single command can open the modal and act on the choice inline — no page or `background.js` listener required. It has no shortcut of its own: bind a palette `command` with a `defaultShortcut` (its action can be the `runScript` that opens the modal, or an `event` a `background.js` listener reacts to). **For large or unbounded lists (a file picker over a big repo), pass a `search(query, { offset, limit })` provider instead of `items`** — Muxy pulls pages on demand per keystroke with a loading spinner and lazy scroll-paging, so nothing is enumerated upfront. `search` may be async on pages/background and is synchronous in `runScript`; it returns `{ items, hasMore }`. See [Modal](https://muxy.app/docs/extensions/modal).
