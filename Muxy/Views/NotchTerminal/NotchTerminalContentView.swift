...
@MainActor
enum NotchTerminalMaterialMask {
...
@MainActor
final class NotchTerminalContentView: NSView {
    static let bridgeHeight: CGFloat = 34

    var onClose: (() -> Void)?
    var onOpenSettings: (() -> Void)?
    var shortcutSettingsProvider: (() -> NotchTerminalShortcutSettingsSnapshot)?
    var onShortcutChange: ((NotchTerminalShortcut) -> String?)?
    ...
    private let shortcutSettingsTitle = NSTextField(labelWithString: "Notch Terminal Shortcut")
    ...
    private var collapsedNotchRect: NSRect?
    ...
    override func layout() {
        ...
        if revealMask.animation(forKey: "notchReveal") == nil {
            revealMask.path = isRevealed ? expandedPath : collapsedPath
        }
    }

    func attach(surface: any NotchTerminalSurface) {
        let view = surface.notchTerminalView
    ...
    func applyAppearance(_ appearance: NotchTerminalAppearance) {
        ...
        terminalBackgroundView.maskImage = NotchTerminalMaterialMask.image(opacity: appearance.blurFraction)
    ...
    func setCollapsedNotchRect(_ rect: NSRect?) {
        collapsedNotchRect = rect
        needsLayout = true
    }
    ...
    func setRevealProgress(_ revealed: Bool) {
        ...
        layer.removeAnimation(forKey: "notchOpacity")
    ...
    func animateReveal(_ revealed: Bool, duration: TimeInterval) {
        ...
        revealMask.add(maskAnimation, forKey: "notchReveal")
        ...
        layer.add(opacityAnimation, forKey: "notchOpacity")
    }
    ...
    private var collapsedPath: CGPath {
        if let collapsedNotchRect {
            let rect = collapsedNotchRect.intersection(bounds)
    ...
    private func configureBridge() {
        ...
        shortcutButton.setAccessibilityLabel("Change notch terminal shortcut")

        configureButton(settingsButton, title: "", symbolName: "gearshape", action: #selector(openSettings))
        settingsButton.setAccessibilityLabel("Open notch terminal settings")
        configureButton(closeButton, title: "", symbolName: "xmark", action: #selector(close))
        closeButton.setAccessibilityLabel("Close notch terminal")
    }
...
struct NotchTerminalShortcutSettingsSnapshot {
    let shortcut: NotchTerminalShortcut
    let monitoringState: NotchTerminalShortcutMonitoringState
