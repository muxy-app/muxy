...
On a display with a physical notch, hovering the notch briefly expands the terminal out of it like a dynamic island. Open **Shortcuts → Notch Terminal** to choose how it also opens from a shortcut:
...
- **Background vibrancy** continuously controls the native macOS material intensity from 0–100%. The notch bridge remains solid.

The vibrancy control mixes the system material continuously; it does not set a custom blur radius.

The shortcut is also available from the shortcut control in the notch terminal. It is stored as `shortcuts.notchTerminal` in `settings.json` using either `{"type":"doubleShift"}` or `{"type":"keyCombo","keyCombo":{"key":"space","modifiers":...},"virtualKeyCode":49}`. Panel dimensions are stored as `muxy.notchTerminal.width` and `muxy.notchTerminal.height`. Glass settings use `muxy.notchTerminal.transparency` as an integer percentage from 0–55 and `muxy.notchTerminal.blur` as an integer material intensity from 0–100.

When macOS Reduce Transparency or Increase Contrast is enabled, Muxy temporarily renders the notch terminal as opaque and unblurred without changing the saved glass settings.
