...
- **Commit** asks you to confirm the commit and push to the active branch, then stages all changes, asks the configured AI CLI for a commit message in the background, and commits and pushes through Muxy's Git service. The provider runs non-interactively with repository-writing tools disabled; it never receives control of the Git workflow. The control is disabled for clean worktrees and detached HEADs.
- **Create PR** appears only after Muxy confirms the branch has no pull request. It is disabled when the working tree is clean. It asks you to confirm before it runs. The configured AI CLI then generates a title, summary, new branch name, and target branch as JSON. Muxy validates that metadata, creates the branch, commits staged changes, pushes, and creates the pull request through its existing GitHub service.
- Both confirmations name the active branch and the provider that will supply the message or pull request text. Switching branch or worktree while a confirmation is open dismisses it, and confirming an action that is no longer available reports that instead of running.
