...
The panel and terminal surface are prepared at their final display-constrained size before they become visible. Show and hide reveal that fixed viewport through a rounded clipping animation instead of resizing the terminal during presentation. The tint is composited below Ghostty, preserving terminal glyph contrast and the active Ghostty theme. The bridge remains solid for readability, and the main window follows the separate **Settings → Interface → Appearance** controls. Muxy uses an opaque, unblurred fallback when macOS Reduce Transparency or Increase Contrast is enabled.
