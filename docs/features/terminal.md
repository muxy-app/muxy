...
Under **Settings -> Composer**, **Clear After Sending** removes the submitted text and attachments only after every
target finishes successfully. If the draft changes while a submission is finishing, Muxy preserves the newer
content. **Clear on Close** removes the draft whenever the Composer closes, including floating auto-close, target
loss, and panel displacement. Both options are off by default.
