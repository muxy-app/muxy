...
| `pane.offline` | `paneID`, `tabID`, `kind`, `projectID`, `worktreeID`, `areaID`, `title`, `projectPath`, `offline`, and optionally `cwd` | `events: ["pane.offline"]` |
...
`pane.offline` fires with `offline: "true"` after Muxy releases an invisible, idle terminal's surface to save memory. It fires with `offline: "false"` after that terminal materializes again. The string value follows the extension event transport format. Extensions can use this event to suspend worktree-scoped resources without polling the terminal screen.

`worktree.offline` aggregates `pane.offline` across one worktree, so an extension holding a worktree-scoped resource (a dev server, a container, a watcher) can subscribe to a single event instead of tracking panes itself. It fires with `offline: "true"` only when **every** terminal pane in the worktree is offline, and with `offline: "false"` again when any pane wakes, when a new terminal pane is created in that worktree, or when the worktree's last terminal pane closes. Those last two cases matter: a sleeping worktree that gains a live terminal — or loses its terminals entirely — is no longer asleep, so a paired `offline: "false"` always follows an `offline: "true"`. It carries `worktreePath` alongside the identifiers, so a `background.js` listener can act on the worktree directly — background scripts have no `muxy.worktrees.list()`. [`examples/demo-docker-sleep`](examples/demo-docker-sleep) is a working listener that stops and starts a worktree's docker compose stack.
