...
Hostnames are resolved before the check, so an alias that points at a private address is blocked too. Redirects are re-validated against the same policy.
