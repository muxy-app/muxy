...
A status bar item is an icon (with optional text) Muxy adds to either side of the footer status bar — the row that shows the active path, repository controls, extension output, and rich-input controls. Clicking it runs one of the extension's declared [commands](palette-commands.md).
