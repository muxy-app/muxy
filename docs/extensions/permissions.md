...
| `agents:read` | `agents.list` — current AI agent status per worktree; `agent.resolveResume` — resolve a fresh resume command for a provider/session/cwd (read-only; it never runs anything). Also required to subscribe to the `agent.status` and `agent.session` [events](events.md). |
