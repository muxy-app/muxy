...
- **Fail-open.** If you register no handler, your handler throws, or it doesn't answer within a few seconds, the close proceeds. A surface can never wedge the close button.
...
- **Toggling a panel or popover closed is a show/hide, not a close** — it bypasses the veto. `onBeforeClose` fires for genuine close intents: the surface's close button, `muxy.panels.close()` / `muxy.popover.close()`, and closing a tab. A topbar/command toggle that hides the surface does not ask.
