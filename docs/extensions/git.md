...
- `worktree.remove` deregisters the worktree while retaining its branch and updates tracked sidebar state for the selected project. Extension-triggered removal runs only pre-authorized per-machine teardown hooks from `$XDG_CONFIG_HOME/muxy/worktree.json` when `XDG_CONFIG_HOME` is non-empty or `~/.config/muxy/worktree.json` otherwise. Project teardown hooks from `<project>/.muxy/worktree.json` require the native removal confirmation that displays every command, so this API skips them. A failed per-machine hook or invalid per-machine configuration blocks removal. Remote and externally managed worktrees do not run local teardown hooks. If a tracked local worktree's main repository is missing, Muxy removes the tracked sidebar entry but preserves all files at the worktree path and returns `dirRemoved: false`, regardless of `force`. If Git no longer registers a tracked Muxy-created checkout in that project's default app-managed worktree directory, `force: true` removes the stale directory only when its `.git` reference proves that it belonged to the selected repository; otherwise Muxy forgets the stale entry but preserves the directory and returns `dirRemoved: false`. Custom, external, cross-project, foreign-repository, and symlinked checkout paths remain protected. Otherwise, with `force: false`, Git performs the final dirty-worktree check and rejects removal when changes remain; pass `force: true` to discard them. `timeoutMs` defaults to 30 seconds, accepts up to 3,600,000 ms (one hour), covers path resolution, teardown, Git removal, stale-directory cleanup, and final verification, and terminates the active process group when exceeded. In the result, `path` is the canonical physical path resolved by Muxy rather than necessarily the caller-supplied representation; `dirRemoved` is `true` when that path is definitely gone, `false` when files remain, and `null` when the final existence check could not be completed within the deadline.
