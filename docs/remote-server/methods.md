...
- `takeOverPane` first replays the pane's buffered raw history as one or more [`terminalOutput`](events.md) events, then pushes a [`terminalSnapshot`](events.md) event, then streams live [`terminalOutput`](events.md) until released. The client must reset its terminal (clear screen and scrollback) before consuming the replay, because the replay always represents the pane's full history from a reset state.
