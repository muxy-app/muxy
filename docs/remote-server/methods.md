...
- `takeOverPane` acknowledges the request before replaying the pane's retained buffer window as one or more [`terminalOutput`](events.md) events. It then pushes a [`terminalSnapshot`](events.md) event and streams live [`terminalOutput`](events.md) until released. The client must reset its terminal (clear screen and scrollback) before consuming the replay, because the replay always represents the bounded retained window (capped by the configured scrollback limit) presented from a reset state.
