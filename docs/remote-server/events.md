...
The client must reset its terminal (clear the screen **and** its own scrollback) before feeding the replay, because the replay always represents the retained buffer window (bounded by the configured scrollback cap) presented from a reset state; otherwise re-entering a pane would duplicate already-rendered history.
